			...
			<GridLayout row="1" colspan="3" rows="*" column="*" width="100%" height="25">
				<StackLayout row="0" col="0" orientation="horizontal" verticalAlignment="bottom" horizontalAlignment="center">
					<StackLayout *ngFor="let indicator of indicators; let i = index">
						<Image width="14" height="14" style.marginRight="10" *ngIf="indicator.active" loadMode="async" src="~/images/icon/indicator_blue_active.png"></Image>
						<Image width="14" height="14" style.marginRight="10" *ngIf="!indicator.active" loadMode="async" src="~/images/icon/indicator_blue_deactive.png" (tap)=GoToSlide(i);></Image>
					</StackLayout>
				</StackLayout>
			</GridLayout>
		</GridLayout>		
...
export class SlidesComponent implements OnInit {
	...
	constructor(private ref: ChangeDetectorRef, public page: Page, private _routerExtensions: RouterExtensions) {
		this.indicators = [];
...
/* setTimeout(() => { 
	...
	public applySwipe(pageWidth: number): void {
		...
		this.currentSlide.slide.layout.on(gestures.GestureTypes.tap, (args): any =>{
				...
				if (args.deltaX > (pageWidth / 8)) {
					if (this.hasPrevious) {
						...
						this.showLeftSlide(this.currentSlide, args.deltaX, endingVelocity).then(() => {
							if (this.currentSlide && this.currentSlide.left) {
				...
				// swiping right to left
					...
					if (this.hasNext) {
						...
						this.showRightSlide(this.currentSlide, args.deltaX, endingVelocity).then(() => {
							if (this.currentSlide && this.currentSlide.right) {
